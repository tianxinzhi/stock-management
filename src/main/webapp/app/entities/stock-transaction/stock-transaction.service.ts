...
export class StockTransactionService {
  ...
  create(stockTransaction: IStockTransaction): Observable<EntityResponseType> {
    return this.http.post<IStockTransaction>(this.resourceUrl, stockTransaction, { observe: 'response' });
  }

  update(stockTransaction: IStockTransaction): Observable<EntityResponseType> {
    return this.http.put<IStockTransaction>(this.resourceUrl, stockTransaction, { observe: 'response' });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IStockTransaction>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<IStockTransaction[]>(this.resourceUrl, { params: options, observe: 'response' });
  }
